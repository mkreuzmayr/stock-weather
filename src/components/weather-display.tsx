...
export function WeatherDisplay({ temperature, condition, cloudCover }: WeatherDisplayProps) {
  ...
  useEffect(() => {
    ...
    const createClouds = () => {
      ...
      clouds.push({
        ...
        shadowColor: isDark ? "rgba(226, 232, 240, 0.2)" : "rgba(244, 63, 94, 0.2)",
        shadowBlur: 30,
        shadowOffsetX: 5,
        shadowOffsetY: 5,
        texture: true,
      ...
      clouds.forEach((cloud) => {
        if (cloud.type === "celestial") {
          ...
          if (cloud.texture) {
            ...
            gradient.addColorStop(0, theme === "dark" ? "rgba(255, 255, 255, 0.2)" : "rgba(255, 255, 255, 0.4)")
          ...
          cloud.bubbles.forEach((bubble: any) => {
            ctx.beginPath()
            ctx.arc(cloud.x + bubble.x, cloud.y + bubble.y, bubble.radius, 0, Math.PI * 2)
            ctx.fillStyle = cloud.color
            ctx.fill()
          })
    ...
    <div className="relative mb-8">
      ...
      <div className="relative z-10 text-center">
        <div className="inline-block">
          <div className="text-[120px] font-bold leading-none tracking-tighter text-gray-900 dark:text-gray-100 drop-shadow-lg">
            {temperature}
          </div>
          <div className="text-2xl font-medium text-center mt-2 text-gray-800 dark:text-gray-200">{condition}</div>
        </div>
      </div>
    </div>
  )
