...
export function WeatherAnimation({ sentiment }: WeatherAnimationProps) {
        ...
        case "neutral": // Cloudy with vibrant colors
          ...
          for (let i = 0; i < 8; i++) {
            particles.push({
              ...
              color: i < 6 ? "rgba(125, 211, 252, 0.9)" : "rgba(56, 189, 248, 0.8)", // Sky-300 and Sky-400
            })
          }
      ...
      particles.forEach((p, index) => {
        switch (p.type) {
          ...
          case "cloud":
            ...
            const drawCloud = (x: number, y: number, size: number, color: string) => {
              ctx.fillStyle = color
              ctx.beginPath()
          ...
          case "corona":
            ...
            p.pulseValue = p.pulseValue !== undefined ? p.pulseValue : Math.PI / 2

            // Animate corona pulsing (opposite phase to the sun)
            p.pulseValue += p.pulseSpeed
            const coronaPulseFactor = 1 + Math.sin(p.pulseValue) * p.pulseAmount
            ...
            const coronaGradient = ctx.createRadialGradient(p.x, p.y, innerRadius, p.x, p.y, outerRadius)
  ...
  return (
    <Card className="mb-6 overflow-hidden border-0 shadow-lg rounded-3xl bg-[#1e1b4b]/80 backdrop-blur-md border border-purple-500/20">
      <div className="relative h-[220px]">
        <canvas ref={canvasRef} className="absolute inset-0 w-full h-full" />
        <div className="absolute inset-0 flex items-center p-6">
          <div className="bg-[#1e1b4b]/70 backdrop-blur-sm p-5 rounded-2xl border border-purple-500/30 shadow-lg shadow-purple-900/20">
            <div className="flex items-center gap-4">
              <div
                className="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br border border-white/20 shadow-inner shadow-white/10"
              ...
              <div>
                <h2
                  className={`text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${sentimentInfo.gradient}`}
                >
                  Market Sentiment
                </h2>
                <p className="text-lg font-medium text-white">{sentimentInfo.text}</p>
                <p className="text-sm text-purple-300/80 mt-1">{sentimentInfo.description}</p>
              </div>
            </div>
          </div>
        </div>
